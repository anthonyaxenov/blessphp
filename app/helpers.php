...
/*
|--------------------------------------------------------------------------
| Additional handy functions you can use anywhere in project
|--------------------------------------------------------------------------
*/
...
{
    $dirname = dirname($_SERVER['DOCUMENT_ROOT']) ?: getcwd();
    return rtrim(sprintf('%s/%s', $dirname, $path), '/');
}
 ...
 */
function _(string $key, array $replacements = []): string
{
    return Flight::get('locale')->get($key, $replacements);
}
